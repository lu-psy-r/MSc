...
For example, you can wrap the `apa()` function around this `cor.test()` command. You can write the following:

`` There was a positive correlation between HLVA and accuracy, `r apa(cor.test(study.two.gen$HLVA, study.two.gen$mean.acc))` ``

To get automatically formatted statistical reports.
