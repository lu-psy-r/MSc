...
```
*(Please note for the part 3 recording, the screen shows PSYC401 instead of PSYC411 in places. I am reluctant to go in and correct this right now to correct the typo as panopto recordings aren't working very well for me at the moment. So, please excuse the error! It* should *be PSYC411)*
...
The data set on the Shipley and Gent vocabulary scores is now updated with the data from your group, so it now contains *nine* years of PSYC411 students' data. I've omitted Age as this might impact anonymity of the data. Download the data from here: [PSYC411-shipley-scores-anonymous-17_25.csv](data/wk4/PSYC411-shipley-scores-anonymous-17_25.csv){target="_blank"} and read the data into an object in R studio called `vdat` (for vocabulary data).

::: {.callout-warning icon=false collapse="false"}
## Answer
``` {r}
library(tidyverse)
vdat <- read_csv("data/wk4/PSYC411-shipley-scores-anonymous-17_25.csv")
```
:::
...
24. Your next task is to run some Chi-squared and Cramer's V tests on some of the other nominal data. Open the data "PSYC411-shipley-scores-anonymous-17_25.csv" again. Investigate the association between gender and year (are there different distributions of males and females in each of our masters' year cohorts) using Chi-squared and Cramer's V. Is it significant? 
...
It is not significant, X-squared = 11.79, df = 14, p-value = .623, but there is < 5 in the 
...
```
It still is not significant, X-squared = 5.99, V = **, df = 7, p-value = .541
...
```
It is not significant, X-squared = 2.09, df = 2, p-value = .352.
...
- [PSYC411-shipley-scores-anonymous-17_24.csv](data/wk4/PSYC411-shipley-scores-anonymous-17_25.csv){target="_blank"}
