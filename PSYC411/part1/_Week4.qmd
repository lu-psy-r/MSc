...
[part 1](ppt/wk4/PSYC411 Lecture week4 part1 handout.pptx), [part 2](ppt/wk4/PSYC411 Lecture week4 part2 handout.pptx), and
...
summary_all_dat_scale <- mutate(summary_all_dat, ahiTotalscale = scale(ahiTotal), cesdTotalscale = scale(cesdTotal))
...
24. Your next task is to run some Chi-squared and Cramer's V tests on some of the other nominal data. Open the data "PSYC401-shipley-scores-anonymous-17_24.csv" again. Investigate the association between gender and year (are there different distributions of males and females in each of our masters' year cohorts) using Chi-squared and Cramer's V. Is it significant? 
...
it isn't significant, X-squared = 10.77, df = 12, p-value = .549, but there is < 5 in the 
...
```
It still isn't significant, X-squared = 5.86, df = 6, p-value = .439
:::
...
```
not significant, X-squared = 1.98, df = 2, p-value = .372.
...
```
Still not significant, X-squared = 0.46, df = 1, p-value = .497
:::
...
PPapoutsi, C., Zimianiti, E., Bosker, H. R., & Frost, R. L. (2024). Statistical learning at a virtual cocktail party. *Psychonomic Bulletin & Review, 31(2)*, 849-861. https://doi.org/10.3758/s13423-023-02384-1

The data are available on OSF, but also a cleaned version of the dataset is [avaiilable here](data/wk4/papatsou_cleaneddata.csv). 
...
Answers will appear in this page, above, after the practical in week 4.
